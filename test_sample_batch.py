...
def test_sample_batch():
    buffer = ReplayBuffer(max_size=1000, chunk_dir="runs/replays/chunks")
    
    # create dummy game
    game = GameHistory()
    game.board_size = 30
    for i in range(100):
        game.actions.append(i % 100)
    ...
    game.winner = 1
    game.target_centers_precomputed = np.zeros((100, 2), dtype=np.int32)
    game.board_snapshots = {0: np.zeros((30, 30), dtype=np.int8)}
    
    buffer.save_game(game)
    print("Game saved to buffer")
    
    # Run once to compile cython if needed
    _ = buffer.sample_batch(batch_size=16, num_unroll_steps=5, td_steps=5, discount=0.997, action_size=441, view_size=21)
